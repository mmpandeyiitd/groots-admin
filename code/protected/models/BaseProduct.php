...
//          }
    ...
    public static function downloadproductCSV() {
        ...
        $sqlchksubsid = "SELECT bp.base_product_id AS 'Subscribed Product ID',title AS Name,sp.store_price AS 'Store Price',sp.store_offer_price AS 'Price(Store Offer Price)','$effective_date' as 'Effective Price Date',bp.image as image,bp.pack_size AS 'Pack Size',bp.pack_unit AS 'Pack Unit',bp.status AS Status,sp.grade AS Grade,sp.diameter AS Diameter, pcm.category_id as categoryId FROM `base_product` `bp` LEFT JOIN `subscribed_product` `sp` ON sp.base_product_id = bp.base_product_id LEFT JOIN product_category_mapping pcm on pcm.base_product_id=bp.base_product_id";
