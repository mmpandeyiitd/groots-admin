    ...
    public function savedatagridview($id, $pro, $ef, $pf, $status) {
        ...
        if ($status == '0') {
         ...
         FROM retailer_product_quotation WHERE retailer_id =$id AND subscribed_product_id =$pro";
            ...
            $command->execute();
            $connection = Yii::app()->db;
            $sql = "INSERT INTO retailer_product_quotation_log(action,effective_price,discount_per,subscribed_product_id,retailer_id,status, date) VALUES('DELETE','$ef', '$pf', '$pro', '$id','$status', NOW())";
            $command = $connection->createCommand($sql);
            $command->execute();
            $connection = Yii::app()->db;
            ...
            $command->execute();
            $connection = Yii::app()->db;
            $sql = "INSERT INTO retailer_product_quotation_log(action,effective_price,discount_per,subscribed_product_id,retailer_id,status,date) VALUES('UPDATE','$ef', '$pf', '$pro', '$id','$status', NOW())";
            $command = $connection->createCommand($sql);
            $command->execute();
        } else {
            $connection = Yii::app()->db;
            ...
            $command->execute();
            $connection = Yii::app()->db;
            $sql = "INSERT INTO retailer_product_quotation_log(action,effective_price,discount_per,subscribed_product_id,retailer_id,status, date) VALUES('INSERT','$ef', '$pf', '$pro', '$id','$status', NOW())";
            $command = $connection->createCommand($sql);
            $command->execute();
            //return $category_id_del= $command->queryAll();
        }
