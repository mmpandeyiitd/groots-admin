...
//        $criteria->compare('buyer_shipping_cost', $this->buyer_shipping_cost);
    ...
    public function GetOrderdetail($order_id) {
      ...
      $sql = "select `ol`.`order_id` AS `order_id`,`ol`.`colour` AS `color`,group_concat(`ol`.`base_product_id` separator ',') AS `base_product_id`,group_concat(`ol`.`subscribed_product_id` separator ',') AS `subscribed_product_id`,group_concat(`ol`.`size` separator ',') AS `size`,group_concat(`ol`.`product_qty` separator ',') AS `qty` ,group_concat(`ol`.`product_name` separator ',') AS `product_name` ,group_concat(`ol`.`unit_price` separator ',') AS `unit_price` ,group_concat(`ol`.`shipping_charges` separator ',') AS `shipping_charges`,group_concat(`ol`.`seller_name` separator ',') AS `seller_name`,group_concat(`sp`.`quantity` separator ',') AS `available_quantity`,group_concat(`bp`.`minimum_order_quantity` separator ',') AS `min_order_quantity`,group_concat(`ol`.`id` separator ',') AS `id` ,group_concat(`ol`.`status` separator ',') AS `status` from `order_line` `ol` left join dev_groots.base_product bp on bp.base_product_id=ol.base_product_id left join `dev_groots`.subscribed_product sp on sp.subscribed_product_id=ol.subscribed_product_id  where ol.order_id=$order_id  group by `ol`.`colour`,`ol`.`product_name`";
