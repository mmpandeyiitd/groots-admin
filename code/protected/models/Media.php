...
class Media extends CActiveRecord {
    ...
    public function getOneMediaremovedefalut($base_product_id) {
        $wsp = 1;

        $connection = Yii::app()->db;
        $sql = "SELECT media_id  FROM `media`  where base_product_id=" . $base_product_id . " AND is_default=0";
        $command = $connection->createCommand($sql);
        $command->execute();
        $img = $command->queryScalar();
        if($img !=''){
        $connection = Yii::app()->db;
        $sql1 = "UPDATE media SET is_default=$wsp,base_product_id =$base_product_id WHERE media_id=" . $img . " AND base_product_id=" . $base_product_id . "";
    ...
    public function updateDefaultMediaByBaseProductId($media_id = null, $base_product_id = null) {
        // echo $media_id;die;
        if (!empty($media_id) AND is_numeric($media_id) AND ! empty($base_product_id) AND is_numeric($base_product_id)) {
