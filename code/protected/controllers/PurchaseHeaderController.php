...
//print_r($_POST);die;
    ...
    public function actionDownloadPurchaseTemplate()
    {
        $w_id = $_GET['w_id'];
        $date = $_GET['date'];
        $sql = 'select ph.id as purchase_id,pl.id,vpm.base_product_id,bp.parent_id, bp.title ,v.name,vpm.vendor_id,"' . $date . '" as date, pl.order_qty, pl.unit_price,pl.price,"" as urd_number from cb_dev_groots.vendor_product_mapping as vpm 
